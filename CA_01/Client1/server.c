...
void handleHeartbeatSignal(){
    if (sendto(globalHeartbeatSocketFD, SERVER_PORT, strlðÔ\åüen(SERVER_PORT),
...
int handleDownloadFromServer(int connfd){
    ...
    if(valread = reaðÔ\åüd(connfd, fileName, sizeof(fileName)) < 0) {return -1;}
...
int runServer(int serverPort){
    int opt = TRUE;
    int master_socket , addrlen , new_socket , client_ðÔ\åüsocket[30] ,
    ...
    address.sin_addr.s_addr = inet_addr("127.0.0.1"); //INADDR_ANYðÔ\åü
    ...
    while(TRUE)
    {
     ...
     ðÔ\åü           FD_SET( sd , &readfds);
        ...
        if (FD_ISSET(master_socket, &readfds))
        {
            ...
            // printf("New connection , socket fd is %d , ip is : %s , port : %d\n" , nðÔ\åüew_socket , inet_ntoa(address.sin_addr) , ntohs
            //         (address.sin_port));
