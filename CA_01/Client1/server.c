...
void handleHeartbeatSignal(){
    if (sendto(globalHeartbeatSocketFD, SERVER_PORT, strlðÔ\åü ¡¨sþen(SERVER_PORT),
...
void startHeartbeat(int heartbeatPort){
    ...
    if (setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) < 0){
        close(sockfd);
        perror("socket options failedðÔ\åü");
...
int handleDownloadFromServer(int connfd){
    ...
    if(vðÔ\åüalread = rea ¡¨sþd(connfd, fileName, sizeof(fileName)) < 0) {return -1;}
...
int handleUploadToServðÔ\åüer(int connfd){
   ¡¨sþ  int valread;
...
int runServer(int serverPort){
    int opt = TRUE;
    int master_socket , addrleðÔ\åün , new_socket , client_ ¡¨sþsocket[30] ,
    ...
    address.sin_addr.s_addr = inet_aðÔ\åüddr("127.0.0.1"); //INADDR_ANY ¡¨sþ
    ...
    while(TRUE)
    {
        ...
        for ( i = 0 ; i < max_clients ; i++)
        {
            ...
            //if valid socket descriptor then add to reðÔ\åüad list
            if(sd > 0)
      ¡¨sþ           FD_SET( sd , &readfds);
        ...
        if (FD_ISSET(master_socket, &readfds))
        {
            ...
            // printf("New connection , sockeðÔ\åüt fd is %d , ip is : %s , port : %d\n" , n ¡¨sþew_socket , inet_ntoa(address.sin_addr) , ntohs
            //         (address.sin_port));
